//
//  DataEntryOrderingAnimalVCIpad.swift
//  SearchPoint
//
...
// MARK: - CLASS
class DataEntryOrderingAnimalVCIpad: UIViewController,UIScrollViewDelegate ,VNDocumentCameraViewControllerDelegate {
    ...
    @IBOutlet weak var scrollInnerHeightConstraint: NSLayoutConstraint!
    ...
    @IBOutlet weak var sireIdAndDamIdStackView: UIStackView!
    ...
    @IBOutlet weak var bckkkBtnOutlet: UIButton!
    ...
    func sideMenuRevealSettingsViewController() -> DataEntryOrderingAnimalVCIpad? {
        var viewController: UIViewController? = self
        
        if viewController != nil && viewController is DataEntryOrderingAnimalVCIpad {
            return viewController! as? DataEntryOrderingAnimalVCIpad
        }
        while (!(viewController is DataEntryOrderingAnimalVCIpad) && viewController?.parent != nil) {
            viewController = viewController?.parent
        }
        if viewController is DataEntryOrderingAnimalVCIpad {
            return viewController as? DataEntryOrderingAnimalVCIpad
        }
        return nil
    }
    ...
    func addBtnCondtion(completionHandler: CompletionHandler){
        if UserDefaults.standard.value(forKey: "DEUSDairyGender") as? String == "F" || UserDefaults.standard.value(forKey: "DEUSDairyGender") as? String == nil {
            ...
            if adata.count > 0{
                ...
                if  msgUpatedd {
                    ...
                    let ok = UIAlertAction(title: "OK", style: .default, handler: { [self] (action) -> Void in
                        if self.dataEntryConflicedBack  {
    ...
    func tableViewpop() {
        ...
        buttonbg2.addTarget(self, action:#selector(DataEntryOrderingAnimalVCIpad.buttonPreddDroper), for: .touchUpInside)
    ...
    func dataPopulateInScreen(animalId:Int){
        ...
        if animalIdBool  {
            ...
            if dataEntryConflicedBack  {
                ...
                self.bckkkBtnOutlet.isHidden = true
            } else {
                dataEntryReviewData.isHidden = false
                self.bckkkBtnOutlet.isHidden = false
