...
class BeefDataEntryReviewDataIpad: UIViewController{
    ...
    func setButtonState(button : UIButton, isOn : Bool) {
        if isOn {
            ...
            button.backgroundColor = UIColor(red: 255/255, green: 92/255, blue: 6/255, alpha: 1.0)
...
//           
//            self.reviewListArray = fetchDataEntryReviewDefaultBeef(entityName: Entities.dataEntryBeefAnimalAddTblEntity,asending:true,userId:userID ?? 0,farmId : self.searchTextField.text ?? "",customerId: currentCustomerId ?? 0,flag: 2, listId: self.listIdGet) as! [DataEntryBeefAnimaladdTbl]
//            self.collectionView.reloadData()
//        }
//        
//        
...
//        dropDown.show()
    ...
    @IBAction func sortByAscending(_ sender: UIButton) {
        if sender.tag == 100{
            ...
            decendingBtnOutlet.backgroundColor = UIColor.init(hex: "FF5C02")
            decendingBtnOutlet.setTitleColor(.white, for: .normal)
            ascendingBtnOutlet.backgroundColor = UIColor.white
            ascendingBtnOutlet.setTitleColor(.black, for: .normal)
            decendingBtnOutlet.layer.borderColor = UIColor(red: 10/255, green: 137/255, blue: 157/255, alpha: 1).cgColor
            if ascendingBtnOutlet.backgroundColor == .white {
                ascendingFound = true
                ascendingBtnOutlet.backgroundColor = UIColor.init(hex: "FF5C02")
                ascendingBtnOutlet.setTitleColor(.white, for: .normal)
                decendingBtnOutlet.backgroundColor = UIColor.white
                decendingBtnOutlet.setTitleColor(.black, for: .normal)
                ascendingBtnOutlet.layer.borderColor = UIColor.white.cgColor
            }
...
//            self.sortByDescendingImgView.image = UIImage(named: "radioSeletedBtn")
            ...
            decendingBtnOutlet.backgroundColor = UIColor.white
            decendingBtnOutlet.setTitleColor(.black, for: .normal)
            ascendingBtnOutlet.layer.borderColor = UIColor(red: 10/255, green: 137/255, blue: 157/255, alpha: 1).cgColor
            if decendingBtnOutlet.backgroundColor == .white {
                ascendingFound = false
                decendingBtnOutlet.backgroundColor = UIColor.init(hex: "FF5C02")
                decendingBtnOutlet.setTitleColor(.white, for: .normal)
                ascendingBtnOutlet.backgroundColor = UIColor.white
                ascendingBtnOutlet.setTitleColor(.black, for: .normal)
                decendingBtnOutlet.layer.borderColor = UIColor.white.cgColor
            }
...
//            self.sortByDescendingImgView.image = UIImage(named: "radioBtn")
    ...
    @IBAction func sortDoneButtonAction(_ sender: UIButton) {
        ...
        if UserDefaults.standard.object(forKey: keyValue.beefDateEntrySaveReviewPreference.rawValue) as? String == keyValue.barcode.rawValue {
            self.reviewListArray = fetchDataEntryReviewDefaultBeef(entityName: Entities.dataEntryBeefAnimalAddTblEntity,asending:ascendingFound,userId:userID ?? 0,farmId : self.searchTextField.text ?? "",customerId: currentCustomerId ?? 0,flag:3, listId: self.listIdGet ?? 0) as! [DataEntryBeefAnimaladdTbl]
            self.collectionView.reloadData()
            
        } else if UserDefaults.standard.object(forKey: keyValue.beefDateEntrySaveReviewPreference.rawValue) as? String == ButtonTitles.earTagText {
           
            self.reviewListArray = fetchDataEntryReviewDefaultBeef(entityName: Entities.dataEntryBeefAnimalAddTblEntity,asending:ascendingFound,userId:userID ?? 0,farmId : self.searchTextField.text ?? "",customerId: currentCustomerId ?? 0,flag: 2, listId: self.listIdGet) as! [DataEntryBeefAnimaladdTbl]
            self.collectionView.reloadData()
        }
    ...
    @IBAction func backBtnNavigateToAnimal(_ sender: Any) {
        ...
        if pviduser == 7 {
            let secondViewController = storyboard.instantiateViewController(withIdentifier: ClassIdentifiers.dataEntryBeefAnimalNZVC) as! DataEntryBeefAnimalNZ_VC
