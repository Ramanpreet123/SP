...
class SettingsVC : UIViewController {
    ...
    //MARK: IB OUTLETS
    @IBOutlet weak var scannerStackViewHeight: NSLayoutConstraint!
    @IBOutlet weak var nominatorStackViewHeight: NSLayoutConstraint!
    @IBOutlet weak var nominatorStackView: UIStackView!
    @IBOutlet weak var keyboardPersistenceStackViewHeight: NSLayoutConstraint!
    @IBOutlet weak var keyboardPersistenceStackView: UIStackView!
    @IBOutlet weak var dobStackView: UIStackView!
    @IBOutlet weak var dobStackViewHeightConstraint: NSLayoutConstraint!
    ...
    var usDairyMainViewHeight = 926.0
    var inheritBeefMainViewHeight = 831.0
    var ausInheritBeefMainViewHeight = 736.0
    ...
    func setUIOnDidLoad(){
        ...
        if fetchSettingData.count == 0 {
            ...
            if pvidDairy == 1{
                self.providerName = "US Dairy Products"
            }
            else if pvidDairy == 2{
                self.providerName = "Uk Dairy Genomics"
            }
            else if pvidDairy == 3 {
                self.providerName = "AU Dairy Products"
            }
            else if pvidDairy == 8 {
                self.providerName = "CLARIFIDE CDCB (BR)"
            }
            else if pvidDairy == 12 {
                self.providerName = "CLARIFIDE CDCB (CAN)"
            }
            else if pvidDairy == 11 {
                self.providerName = "CLARIFIDE CDCB (BeNeLux)"
            }
            else if pvidDairy == 10 {
                self.providerName = "CLARIFIDE CDCB (IT)"
            }
            else {
                self.providerName = "CLARIFIDE Girolando (BR)"
            }
            ...
            self.providerName = fetchDat?.providerName ?? ""
            ...
            } else if (fetchDat?.speciesName == "Beef" && fetchDat?.providerName == "US Dairy Products") {
    ...
    func setUIOnWillAppear(){
        ...
        if UserDefaults.standard.string(forKey: "name") == "Dairy"{
            ...
            if  UserDefaults.standard.integer(forKey:keyValue.providerID.rawValue ) == 1 ||  UserDefaults.standard.integer(forKey:keyValue.providerID.rawValue ) == 2 ||  UserDefaults.standard.integer(forKey:keyValue.providerID.rawValue ) == 3 || UserDefaults.standard.integer(forKey:keyValue.providerID.rawValue ) == 8 || UserDefaults.standard.integer(forKey:keyValue.providerID.rawValue ) == 10 || UserDefaults.standard.integer(forKey:keyValue.providerID.rawValue ) == 11 || UserDefaults.standard.integer(forKey:keyValue.providerID.rawValue ) == 12 {
                ...
                self.mainViewHeightConstraint.constant = usDairyMainViewHeight
                if getListProvider.count == 0 {
                    marketTipYopOutlet.isHidden = true
                    providerTitleLbl.isHidden = true
                } else {
                ...
                if UserDefaults.standard.integer(forKey:keyValue.providerID.rawValue ) == 11 {
                    ...
                    self.mainViewHeightConstraint.constant = inheritBeefMainViewHeight
    ...
    func setUpLandscapeConstraints(){
        dobStackViewHeightConstraint.constant = 95
        keyboardPersistenceStackViewHeight.constant = 95
        scannerStackViewHeight.constant = 95
        nominatorStackViewHeight.constant = 95
        nominatorStackView.axis = .horizontal
        nominatorStackView.spacing = 15
        if self.speciesName == "Dairy" && (self.providerName == "US Dairy Products" || self.providerName == "AU Dairy Products" || self.providerName == "CLARIFIDE CDCB (BR)" || self.providerName == "CLARIFIDE CDCB (CAN)" || self.providerName == "CLARIFIDE CDCB (IT)"){
            if mainViewHeightConstraint.constant > 926 {
                if nominatorView.isHidden {
                    usDairyMainViewHeight = usDairyMainViewHeight - 405
                } else {
                    usDairyMainViewHeight = usDairyMainViewHeight - 540
                }
                mainViewHeightConstraint.constant = usDairyMainViewHeight
            }
        }
        
        else if self.speciesName == "Dairy" && (self.providerName == "CLARIFIDE CDCB (BeNeLux)" || self.providerName == "UK Dairy Genomics") {
            if mainViewHeightConstraint.constant > 831 {
                inheritBeefMainViewHeight = inheritBeefMainViewHeight - 405
                mainViewHeightConstraint.constant = inheritBeefMainViewHeight
            }
        }
        
        else if (self.speciesName == "Dairy" && self.providerName == "CLARIFIDE Girolando (BR)") {
            if mainViewHeightConstraint.constant > 831 {
                ausInheritBeefMainViewHeight = ausInheritBeefMainViewHeight - 270
                mainViewHeightConstraint.constant = ausInheritBeefMainViewHeight
            }
        }
        
        else if (self.speciesName == "Beef" && self.providerName == "US Dairy Products" || self.providerName == "AU Dairy Products" || self.providerName == "CLARIFIDE CDCB (BR)") {
            if mainViewHeightConstraint.constant > 831 {
                if !scannerViewShow.isHidden {
                    inheritBeefMainViewHeight = inheritBeefMainViewHeight - 270
                    mainViewHeightConstraint.constant = inheritBeefMainViewHeight
                } else {
                    ausInheritBeefMainViewHeight = ausInheritBeefMainViewHeight - 270
                    mainViewHeightConstraint.constant = ausInheritBeefMainViewHeight
        ...
        dobStackView.spacing = 15
        dobStackView.axis = .horizontal
        keyboardPersistenceStackView.spacing = 15
        keyboardPersistenceStackView.axis = .horizontal
        scannerStackView.spacing = 15
        scannerStackView.axis = .horizontal
    }
    
    func setUpPortraitConstraints(){
        dobStackViewHeightConstraint.constant = 230
        keyboardPersistenceStackViewHeight.constant = 230
        scannerStackViewHeight.constant = 230
        if self.speciesName == "Dairy" && (self.providerName == "US Dairy Products" || self.providerName == "CLARIFIDE CDCB (BR)" ) {
            if !nominatorView.isHidden {
                nominatorStackViewHeight.constant = 230
                nominatorStackView.axis = .vertical
                nominatorStackView.spacing = 40
                usDairyMainViewHeight = usDairyMainViewHeight + 540
            } else {
                nominatorStackViewHeight.constant = 95
                nominatorStackView.axis = .horizontal
        ...
        else if self.speciesName == "Dairy" && (self.providerName == "AU Dairy Products" || self.providerName == "CLARIFIDE CDCB (CAN)" || self.providerName == "CLARIFIDE CDCB (IT)") {
            nominatorStackViewHeight.constant = 95
            nominatorStackView.axis = .horizontal
            nominatorStackView.spacing = 15
            usDairyMainViewHeight = usDairyMainViewHeight + 405
        ...
        dobStackView.axis = .vertical
        dobStackView.spacing = 40
        keyboardPersistenceStackView.spacing = 40
        keyboardPersistenceStackView.axis = .vertical
        scannerStackView.spacing = 40
        scannerStackView.axis = .vertical
    }
    ...
    func getMarketsForCurrentCustomer() {
        if let currentCustomerId = UserDefaults.standard.value(forKey: "currentActiveCustomerId") as? Int {
            ...
            let marketCodes = markets.compactMap { ($0 as AnyObject).marketCode }

                // Example: Print all market codes
                print("Market Codes:", marketCodes)
                
                // If you just need the first one:
                if let firstMarketCode = marketCodes.first {
                    print("First Market Code:", firstMarketCode as Any)
                    self.marketCode = firstMarketCode ?? ""
                }
    ...
    @objc func orientationChanged() {
        self.buttonbg1.removeFromSuperview()
        ...
        if specisObject.speciesName ==  "Dairy" {
            ...
            if UserDefaults.standard.integer(forKey:keyValue.providerID.rawValue ) == 1 ||  UserDefaults.standard.integer(forKey:keyValue.providerID.rawValue ) == 2 ||  UserDefaults.standard.integer(forKey:keyValue.providerID.rawValue ) == 3
                ...
                if UserDefaults.standard.integer(forKey:keyValue.providerID.rawValue ) == 2{
                    ...
                    self.primarlybasedStackView.isHidden = true
                    self.continueOrderBottomConstraint.constant = 66.0
                    self.mainViewHeightConstraint.constant = 831
                }
            }
            else {
                ausInheritBeefMainViewHeight = 736.0
                self.keyBaordFullView.isHidden = true
                keyBoardTitleView.isHidden = true
                alphaNumericbtnOutler.isHidden = true
                scannerViewShow.isHidden = true
            ...
            if UserDefaults.standard.string(forKey: "providerNameUS") == "CLARIFIDE CDCB (US)" || UserDefaults.standard.string(forKey: "providerNameUS") == "US Dairy Products"{
                ...
                if UserDefaults.standard.integer(forKey:keyValue.providerID.rawValue ) == 1 {
                    nominatorView.isHidden = false
                } else {
                    nominatorView.isHidden = true
                }
            ...
            inheritBeefMainViewHeight = 831.0
            ausInheritBeefMainViewHeight = 736.0
            self.speciesName = "Beef"
            if sender.layer.borderColor == UIColor.clear.cgColor {
                return
        ...
        if UserDefaults.standard.string(forKey: "name") == "Dairy" {
            ...
            if pviduser != providerObject.providerId {
                ...
                alert.addAction(UIAlertAction(title:  NSLocalizedString("Yes", comment: ""),style: UIAlertAction.Style.default, handler: { (action: UIAlertAction!) in
                    ...
                    if pvid == 8 {
                        self.ausInheritBeefMainViewHeight = 736.0
                    ...
                    if animaltbl1.count > 0 || animaltbl.count > 0 {
                        ...
                        alert.addAction(UIAlertAction(title: NSLocalizedString("YES", comment: ""), style: UIAlertAction.Style.default,
                            ...
                            self.usDairyMainViewHeight =  926
                            self.orientationChanged()
                        }))
                    ...
                    if providerObject.providerId == 1 || providerObject.providerId == 3 || providerObject.providerId == 8{
                        ...
                        self.primarlybasedStackView.isHidden = false
                        self.continueOrderBottomConstraint.constant = 26.0
                        self.mainViewHeightConstraint.constant = self.usDairyMainViewHeight
                    } else {
                    ...
                    self.usDairyMainViewHeight =  926
                    self.providerName = providerObject.providerName ?? ""
                    self.orientationChanged()
