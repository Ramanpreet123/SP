...
class ResolveIssuesiPadVC: UIViewController{
    ...
    @IBOutlet weak var sortByBttn: UIButton!
    ...
    func setUIOnDidLoad(){
        ...
        let singleTime = fetchAllSingleTimeToastSetting(entityName: Entities.singleTimeToastTblEntity,emailId: email.lowercased()) as! [SingleTimeToastAction]
    ...
    func setUIOnWillAppear(){
        ...
        self.filter2Array = [NSLocalizedString(LocalizedStrings.showParentageCorr, comment: ""),NSLocalizedString(LocalizedStrings.showGenomicCorr, comment: ""),NSLocalizedString(LocalizedStrings.showAllStr, comment: "")]
    ...
    func sideMenuRevealSettingsViewController() -> ResolveIssuesiPadVC? {
        var viewController: UIViewController? = self
        
        if viewController != nil && viewController is ResolveIssuesiPadVC {
            return viewController! as? ResolveIssuesiPadVC
        }
        while (!(viewController is ResolveIssuesiPadVC) && viewController?.parent != nil) {
            viewController = viewController?.parent
        }
        if viewController is ResolveIssuesiPadVC {
            return viewController as? ResolveIssuesiPadVC
        }
        return nil
    }
    ...
    func setUpLandscapeConstraints(){
        print("Landscape")
        self.searchButtonTrailingConstraint.constant = 20
        self.stackViewWidthConstraint.constant = 860
    }
     
    func setUpPortraitConstraints(){
        print("Portrait")
        self.searchButtonTrailingConstraint.constant = 20
        self.stackViewWidthConstraint.constant = 560
    }
    ...
    @IBAction func sortByDoneAction(_ sender: UIButton) {
        ...
        self.backgroundBttn.isHidden = true
    }
    ...
    @IBAction func sortByOrderIdAction(_ sender: UIButton) {
        let userId = UserDefaults.standard.integer(forKey: keyValue.userId.rawValue)
        if sortByOrderIDImgView.image == UIImage(named: "radioSeletedBtn") {
            return
        }
    ...
    @IBAction func sortByAction(_ sender: UIButton) {
        ...
        self.backgroundBttn.isHidden = false
