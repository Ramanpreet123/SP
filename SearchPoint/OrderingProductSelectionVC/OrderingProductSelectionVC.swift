//
//  OrderingProductSelectionVC.swift
//  SearchPoint
//
...
class OrderingProductSelectionVC: UIViewController{
  ...
  func setUIOnWillAppear(){
    ...
    if UserDefaults.standard.value(forKey: keyValue.nominatorSave.rawValue) == nil || UserDefaults.standard.value(forKey: keyValue.nominatorSave.rawValue) as? String == ""{
      ...
      var nomi = UserDefaults.standard.value(forKey: keyValue.nominatorSave.rawValue) as? String
      nominatorLbl.text = "     " + (nomi ?? "")
    }
...
private typealias DeleteDataListHelper = OrderingProductSelectionVC
