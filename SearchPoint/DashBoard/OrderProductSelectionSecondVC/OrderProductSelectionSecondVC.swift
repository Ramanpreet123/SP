...
class OrderProductSelectionSecondVC: UIViewController {
    ...
    func deleteAnimal(productArray : [ProductAdonAnimalTbl], cartAnimalArray : [AnimaladdTbl]){
        ...
        for i in 0...cartAnimalArray.count - 1  {
            ...
            var animalDetail = cartAnimalArray[i]
    ...
    func isSelectedBaseProductCompatiablewithBVDV(indexPath: IndexPath,userId :Int,orderId: Int, collectionViewTag: Int, _ collectionView: UICollectionView)  {
      for i in 0..<self.selection.count{
        if let value = self.selection[i]["animalId"] as? Int, value  == self.aTag{
          let adonId = ( (self.selection[i]["addon"] as! NSArray).object(at: indexPath.row - 2) as! SubProductTbl).adonName
          if adonId == "BVDV" {
            // FIRSTLY CHECK ALL BASE PRODUCT ARE COMPTAIBLE WITH bvdv
            // if yes - "BVDV will be applied to the entire order"
            // if no - "BVDV must be applied to the entire order. One of the products you selected not compatible with BVDV as an add-on. Please change the base product or remove animal from order"
            ...
            let okAction = UIAlertAction(title: NSLocalizedString("Continue", comment: ""), style: UIAlertAction.Style.default) {
              ...
              //Non BVDV SELECTION PROCESS Like CVM, Heridity
              let alertController = UIAlertController(title: NSLocalizedString("Alert", comment: ""), message: "BVDV will be applied to the entire order.".localized, preferredStyle: .alert)
...
//    if UserDefaults.standard.bool(forKey: "BVDVSeleted") && pid == 3{
//      let animalID =  animaltag
//      let data12333 = fetchSubProductDataDairy(productId: 3,animalTag: animalID, orderId: orderId,userId:userId) as! [SubProductTbl]
//      
//      if data12333.count == 0{
//        updateProductTablaid(entity: "ProductAdonAnimalTbl", productId: pid, status: "false")
//      }
//      let productID = pid
//      
//      
//      //Temp Code Rajni 20Aug
//      
//      for i in 0..<data12333.count{
//        
//        let subproductData = fetchSubProductDataDairy(productId: Int(3),animalTag: animalID,orderId:orderId,userId:userId) as? [SubProductTbl]
...
//          updateAdonDataSingleanimalId(entity: "SubProductTbl", pid:3,adonId: Int(subproductData?[i].adonId ?? 0) , status: "false", animalId:Int(subproductData?[i].animalId ?? 0),ordeId:orderId,userId: userId)
//        }
//        for i in 0..<self.selection.count{
