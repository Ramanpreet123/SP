...
//MARK: CLASS
class BeefViewAnimalVC: UIViewController {
    ...
    @IBAction func emailOrder(_ sender: UIButton) {
        ...
        if fetchDataEntry.count > 0 {
            ...
            self.objApiSync.postListDataBeef(listId:Int64(fetchDataEntry[0].listId),custmerId:Int64(custmerId ?? 0),listName: listName)
    ...
    @IBAction func backBtnAction(_ sender: UIButton) {
        if viewAnimalArray.count == 0 {
            ...
            let storyBoard: UIStoryboard = UIStoryboard(name: StoryboardType.MainStoryboard, bundle: nil)
            let newViewController = storyBoard.instantiateViewController(withIdentifier: ClassIdentifiers.beefOrderProductSelectionReviewVC) as! BeefOrderProductSelectionReviewVC
            self.navigationController?.pushViewController(newViewController, animated: false)
        }
        else if productBackSave ==  true {
            let storyBoard: UIStoryboard = UIStoryboard(name: StoryboardType.MainStoryboard, bundle: nil)
            let newViewController = storyBoard.instantiateViewController(withIdentifier: ClassIdentifiers.beefOrderProductSelectionSecondVC) as! BeefOrderProductSelectionSecondVC
            self.navigationController?.pushViewController(newViewController, animated: false)
        }
        else {
...
extension BeefViewAnimalVC : offlineCustomView {
    func crossBtnCall() {
        buttonbg.removeFromSuperview()
        customPopView.removeFromSuperview()
    }
