...
//        }
    ...
    private func initializeHeaders() -> [String : String] {
       let headers = [
        "Keys.headerToken": self.bearerToken
       ]
       return headers
    }
...
extension APIService {
    ...
    func buildRequest(with method: HttpMethod, url: URL, parameters: [String: Any]?, files: [File]?) -> URLRequest {
        ...
        case .GET, .DELETE:
            ...
            request?.addValue(LocalizedStrings.appJson, forHTTPHeaderField: LocalizedStrings.contentType)
            request?.addValue("Basic cGVhc3lBcHA6QDp7cGVhc3khI0FwcC09XiF9", forHTTPHeaderField: LocalizedStrings.authorizationHeader)
            var selectedLanguage = "1"
            if let languageVal = UserDefaults.standard.value(forKey: "LANGUAGE_INDEX") as? String {
                selectedLanguage = languageVal
            }
            
            request?.addValue(selectedLanguage, forHTTPHeaderField: "languageselected")
//            request?.addValue("2", forHTTPHeaderField: "languageselected")
            let fcmToken = UserDefaults.standard.value(forKey: "FCM_TOKEN") as? String ?? ""
            request?.addValue(fcmToken, forHTTPHeaderField: "devicetoken")
        ...
        print("Header is  \(String(describing: request?.allHTTPHeaderFields))")
    ...
    func buildRequestForMultiMedia(_ serviceURL: URL, method: HttpMethod, multimediaFiles: [File]?, parameters: [String: Any]?) -> URLRequest {
        ...
        if let images = multimediaFiles, images.count > 0 {
            ...
            uploadRequest.addValue("multipart/form-data; boundary=" + boundary, forHTTPHeaderField: LocalizedStrings.contentType)
            ...
            uploadRequest.addValue(LocalizedStrings.appJson, forHTTPHeaderField: LocalizedStrings.contentType)
            uploadRequest.addValue("Basic cGVhc3lBcHA6QDp7cGVhc3khI0FwcC09XiF9", forHTTPHeaderField: LocalizedStrings.authorizationHeader)
            var selectedLanguage = "1"
            if let languageVal = UserDefaults.standard.value(forKey: "LANGUAGE_INDEX") as? String {
                selectedLanguage = languageVal
            }
                        
            uploadRequest.addValue(selectedLanguage, forHTTPHeaderField: "languageselected")
//            uploadRequest.addValue("2", forHTTPHeaderField: "languageselected")
            let fcmToken = UserDefaults.standard.value(forKey: "FCM_TOKEN") as? String
            uploadRequest.addValue(fcmToken ?? "", forHTTPHeaderField: "devicetoken")
            
            uploadRequest.addValue("2", forHTTPHeaderField: "platform")
...
extension APIService {
    func handleResponse<T: Decodable>(data: Data, response: URLResponse?, modelType: T.Type, completion: @escaping (Result<T?>) -> Void) {
        ...
        if 200 ... 299 ~= statusCode {
            ...
            print("ERROR IN SERVICE : \(error.localizedDescription)")
            ...
            if statusCode == 401 {
                ...
                DispatchQueue.main.async {
                    ...
                    let configAlert: AlertUI = ("", "Your session has been expired. Please try to login again!")
