//
//  InheritBeefVCInheritMethods.swift
//  SearchPoint
//
...
//            } else {
        ...
        self.inheritEarTagView.backgroundColor = UIColor.white
    ...
    func inheritAddBtnCondtion(completionHandler: CompletionHandler){
        ...
        let animalfilter = barCode.filter({ $0.animalbarCodeTag == inheritBarcodeTextfield.text})
//        if animalfilter.count > 0 {
...
//                    inheritDobBttn.layer.borderWidth = 0.5
        ...
        if beefAnimalData.count > 0 {
          ...
          if existAnimalData.orderstatus == "true" {
            if existAnimalData.date != dateVale || existAnimalData.breedId != breedId || existAnimalData.gender?.localized != inheritGenderString || autoPopulateAnimalData?.animalTag != inheritEarTagTextfield.text {
        ...
        let animalData = fetchAnimaldataValidateAnimalBarcode(entityName: Entities.beefAnimalAddTblEntity, animalbarCodeTag:incrementalBarCode, orderId: orderId, userId: userId)
        ...
        //|| isAnimalComingFromCart, changed for auto populate inherit beef ticket
