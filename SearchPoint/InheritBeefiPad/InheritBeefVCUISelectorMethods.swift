//
//  InheritBeefVCUISelectorMethods.swift
//  SearchPoint
//
...
extension InheritBeefVC {
    ...
    @objc func buttonPreddDroper() {
        ...
        self.inheritGenderView.layer.borderColor = UIColor(red: 57/255, green: 69/255, blue: 73/255, alpha: 1).cgColor
...
extension InheritBeefVC {
    ...
    func byDefaultSetting(){
        ...
        if UserDefaults.standard.string(forKey: keyValue.beefBreed.rawValue) == nil{
            let inheritBreed = fetchAllDataProductBeefId(entityName: Entities.getBreedsTblEntity, breedName: "", productId: 19)
        ...
        if !isautoPopulated {
            ...
            if UserDefaults.standard.bool(forKey: keyValue.isBarCodeIncremental.rawValue) {
                if let lastSavedBarCode = UserDefaults.standard.value(forKey: keyValue.barcodeIncremental.rawValue) as? String {
                    ...
                    inheritBarcodeTextfield.text = isBarcodeEndingWithNumberAndGetIncremented(lastSavedBarCode).incrementedBarCode
                }
            }
        } else {
    ...
    func tableViewpop() {
        ...
        buttonbg2.addTarget(self, action:#selector(InheritBeefVC.buttonPreddDroper), for: .touchUpInside)
    ...
    func setUIOnDidLoad(){
        ...
        if UserDefaults.standard.string(forKey: keyValue.beefProduct.rawValue) != keyValue.globalHD50K.rawValue {
            if isautoPopulated != true {
                if UserDefaults.standard.bool(forKey: keyValue.showbeefInheritTable.rawValue) && animalCount1.count == 0{
    ...
    func setUIOnWillAppear(){
        ...
        inheritDateTextField.addPadding(.left(20))
        ...
        if UserDefaults.standard.integer(forKey: keyValue.beefAnimalIdSelectionCart.rawValue) > 0{
            let cartAnimalID = UserDefaults.standard.integer(forKey: keyValue.beefAnimalIdSelectionCart.rawValue)
