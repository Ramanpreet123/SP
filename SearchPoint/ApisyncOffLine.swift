...
class ApisyncOffLine :NSObject {
    ...
    func tempBeef(data:OfflineOrderBeef){
        ...
        for item in animaltbl {
                ...
                if UserDefaults.standard.integer(forKey: keyValue.beefPvid.rawValue) == 5  {
                    if UserDefaults.standard.string(forKey: keyValue.beefProduct.rawValue) == keyValue.globalHD50K.rawValue {
                        ...
                        if value.breedId != ""{
                            ...
                            ob.breedId = value.breedId //?? "d352c4c2-2ff9-451a-9c00-4f0f5604b387"
            ...
            Apisync.animals.append(ob)
            ob = AnimalDetail()
        }
        let custId = UserDefaults.standard.integer(forKey: keyValue.currentActiveCustomerId.rawValue)
        Apisync.customerId = String(custId)
        Apisync.billToCustomerId = String(billToCustomerId)
        Apisync.species = marketNameType.Beef.rawValue
        Apisync.isOFFline = true
        if beefpvid == 5 {
            ...
            if  selectedPrimaryBreed != ""{
                Apisync.breed1 = selectedPrimaryBreed
            }
            ...
            if  selectedSecondaryBreed != "" {
                Apisync.breed2 = selectedSecondaryBreed
            }
            ...
            if  selectedTertiaryBreed != ""  {
                Apisync.breed3 = selectedTertiaryBreed
            }
            
        }
        Apisync.nominatorId = 0
        Apisync.providerId = beefpvid
        
        
        do {
            let jsonEncoder = JSONEncoder()
            let jsonData = try! jsonEncoder.encode(Apisync)
    ...
    func temp(data:OfflineOrder){
                    ...
                    if let productValue = item as? ProductAdonAnimalTbl{
                        ...
                        for item in addonId {
                            if let addOnValue = item as? SubProductTbl{
                                // if addOnValue.farmId! == productValue.farmId!{
                                addOn.addOnID = addOnValue.adonId
                                product.addOns.append(addOn)
                            }
                        }
        ...
        Apisync.customerId = String(custId)
        Apisync.billToCustomerId = String(billToCustomerId)
        Apisync.species = marketNameType.Dairy.rawValue
        Apisync.isOFFline = true
        Apisync.nominatorId = nominatorid
        Apisync.providerId = pvid
    ...
    func productSave(orderId:Int,animalTag:String){
        ...
        do {
            ...
            let urlString: String = Configuration.Dev(packet: ApiKeys.productSave.rawValue).getUrl()
            var request = URLRequest(url: URL(string: urlString)! )
            request.httpMethod = "POST"
            request.allHTTPHeaderFields = headerDict
            request.setValue(LocalizedStrings.appJson, forHTTPHeaderField: LocalizedStrings.contentType)
    ...
    func multipleAdonSave(orderId:Int,animalTag:String){
        ...
        do {
            ...
            let urlString: String = Configuration.Dev(packet: ApiKeys.adonSave.rawValue).getUrl()
            var request = URLRequest(url: URL(string: urlString)! )
            request.httpMethod = "POST"
            request.allHTTPHeaderFields = headerDict
            request.setValue(LocalizedStrings.appJson, forHTTPHeaderField: LocalizedStrings.contentType)
