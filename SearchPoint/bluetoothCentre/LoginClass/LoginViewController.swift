...
class LoginViewController: UIViewController,UITextViewDelegate{
    ...
    override func viewDidLoad() {
        ...
        if contactSuppportArray!.count == 0 {
            contactVM = ContactSupportViewModel(ref: self, callBack: navigaetE)
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        let firstLogin = UserDefaults.standard.value(forKey: keyValue.firstLogin.rawValue) as? String
        
        if UserDefaults.standard.value(forKey: keyValue.accessToken.rawValue) is String
        {
            if UserDefaults.standard.bool(forKey: keyValue.mustRegisterDevice.rawValue) == false {
        ...
        if UserDefaults.standard.bool(forKey: keyValue.terms.rawValue) == false {
            if let accessToken =   UserDefaults.standard.value(forKey: keyValue.accessToken.rawValue) as? String {
                let mustValidateDevice = UserDefaults.standard.bool(forKey: keyValue.mustValidateDevice.rawValue)
                let mustRegisterDevice = UserDefaults.standard.bool(forKey: keyValue.mustRegisterDevice.rawValue)
                
                if mustValidateDevice || mustRegisterDevice {
...
//            let viewController = storyboard.instantiateViewController(withIdentifier: ClassIdentifiers.dashboardVC) as! DashboardVC
...
//        }
        ...
        self.loginScreenSetViewModel =  LoginScreenSetViewModel(ref: self, callBack: callRegisterDevice)
    }
...
//        }
    ...
    fileprivate func gigyaLoginSetup() {
        ...
        var newParam = ["startScreen": "gigya-login-screen", "lang" : langCode]
        gigya.logout()
        Gigya.sharedInstance().showScreenSet(with: screenSet, viewController: self , params: newParam) { [weak self] (result) in
            ...
            print(result)
                ...
                case .onLogin(let account):
                    ...
                    uID = account.UID ?? ""
                    uIDSign = account.UIDSignature ?? ""
                    signTimeStamp = account.signatureTimestamp ?? ""
...
//        hideIndicator()
//        self.view.isUserInteractionEnabled = true
//        if dataViewModel.modalObject?.mustValidateDevice == true || dataViewModel.modalObject?.mustRegisterDevice == true {
