//
//  OrderingAnimalVCiPadAddAnimalFlow.swift
//  SearchPoint
//
...
extension OrderingAnimalipadVC {
    ...
    func addAnimalBtn_raman(completionHandler: @escaping CompletionHandler){
            ...
            if newcheck.count > 0{
                ...
                alert.addAction(UIAlertAction(title: NSLocalizedString(LocalizedStrings.overWriteText, comment: ""), style: .default, handler: { [self]  (_) in
                    ...
                    if self.farmIDBoolEntryTag {
                        ...
                        if bbb.count != 0 &&  self.dataAutoPopulatedBool{
                    ...
                    if self.farmIDBoolEntrySecond {
                        ...
                        if animalTag1.count != 0 &&  self.dataAutoPopulatedBool {
                    ...
                    if valueCheck!.count != 0{
                        self.dateBtnOutlet.layer.borderColor = UIColor(displayP3Red: 57/255, green: 69/255, blue: 73/255, alpha: 1).cgColor
                    ...
                    if self.damtexfield.text!.count == 0 {
                        self.damtexfield.layer.borderColor = UIColor(displayP3Red: 57/255, green: 69/255, blue: 73/255, alpha: 1).cgColor
                    }
                    if self.sireIdTextField.text!.count == 0 {
                        self.sireIdTextField.layer.borderColor = UIColor(displayP3Red: 57/255, green: 69/255, blue: 73/255, alpha: 1).cgColor
                    }
                    if self.scanBarcodeText.text!.count == 0 {
                        self.barcodeView.layer.borderColor = UIColor(displayP3Red: 57/255, green: 69/255, blue: 73/255, alpha: 1).cgColor
                    }
                    if !self.validateDateFlag || self.dateTextField.text?.count != 10{
                    ...
                    if self.scanAnimalTagText.tag == 1{
                        if self.scanAnimalTagText.text == "" || self.scanBarcodeText.text == ""  {
                            ...
                            if self.sireIdTextField.text?.count != 0 || self.damtexfield.text?.count != 0 {
                                ...
                                if self.borderRedCheck || self.damIdValidationB || self.sireIdValidationB  {
                                    
                                    if self.sireIdTextField.text?.count != 0{
                                        if self.sireIdValidationB  {
                                            if self.pvid == 3 {
                                                if self.ausBullId.contains(self.sireIdTextField.text?.uppercased() as Any) || self.sireNationalID.contains(self.sireIdTextField.text?.uppercased() as Any){
        ...
        else if !self.validateDateFlag || self.dateTextField.text?.count != 10{
            if self.dateTextField.text?.count == 10 {
                let validate = self.isValidDate(dateString: self.dateTextField.text ?? "")
                if validate == LocalizedStrings.correctFormatStr {
                    self.validateDateFlag = true
    ...
    func addAnimalBtn(completionHandler: @escaping CompletionHandler){
                        ...
                        if self.scanAnimalTagText.text == "" || self.scanBarcodeText.text == ""  {
                            ...
                            if UserDefaults.standard.value(forKey: "isAuSelected") as? String == "True" {
                                ...
                                if self.farmIdTextField.text?.count == 0 {
                                    ...
                                    // removed code for new condition
        ...
        if dateBtnOutlet.titleLabel?.text == "MM/DD/YYYY" ||  dateBtnOutlet.titleLabel?.text == "DD/MM/YYYY" {
        ...
        if valueCheck!.count != 0{
            ...
            calenderDobView.layer.borderColor = UIColor(displayP3Red: 57/255, green: 69/255, blue: 73/255, alpha: 1).cgColor
            ...
            if existAnimalData?.orderstatus == "true" {
                if existAnimalData?.date != dateVale || existAnimalData?.eT != etBtn || existAnimalData?.breedId != breedId || existAnimalData?.gender?.localized != genderString || existAnimalData?.farmId != farmIdValue || existAnimalData?.animalTag != animalTagValue {
                    ...
                    let okAction = UIAlertAction(title: NSLocalizedString("OK", comment: ""), style: UIAlertAction.Style.default) { [self]
                        UIAlertAction in
                        let animalFetch = fetchAnimaldataValidateAnimalwithouOrderID(entityName: Entities.animalMasterTblEntity, animalTag:existAnimalData?.animalTag ?? "", farmId: existAnimalData?.farmId ?? "", animalbarCodeTag: scanBarcodeText.text!.uppercased(), offPermanentId: "", offDamId: self.damtexfield.text!.uppercased(), offsireId: self.sireIdTextField.text!.uppercased(),orderId:orderId,userId:userId,custmerId:self.custmerId)
                        self.autoPop(animalData: animalFetch)
                        self.barcodeEnable = false
                    }
                    alertController.addAction(okAction)
        ...
        if animalData.count > 0 && filterbarcodeAnimal.count > 0 || animalData.last?.animalbarCodeTag == ""  || filteranimalMasterData.count > 0 {
            ...
            if dataBVDV.count>0 {
                if  tissueBtnOutlet.titleLabel!.text! == ButtonTitles.allflexTSUText || tissueBtnOutlet.titleLabel!.text! == ButtonTitles.allflexTSTText || tissueBtnOutlet.titleLabel!.text! == ButtonTitles.caisleyTSUText{
                    isUpdate = true
                    updateOrderStatusISyncAnimalMaster(entity: Entities.animalMasterTblEntity, animalTag: scanAnimalText, barCodetag: scanBarcodeText.text!, date: dateVale , damId: damtexfield.text!.uppercased(), sireId: sireIdTextField.text?.uppercased() ?? "" , gender: genderString,update: "false", permanentId: "", tissuName: sampleName, breedName: breedBtnOutlet.titleLabel!.text!, et: etBtn, farmId:farmIdText, orderId: autoD,orderSataus:"false",breedId:breedId,isSync:"false", providerId: pvid,tissuId: sampleID,sireIDAU: "", nationHerdAU: "", userId: userId,udid:timeStampString,animalId: animalId1, selectedBornTypeId: selectedBornTypeId,isSyncServer:false, adhDataServer: false, editFlagSave: true, territoryGeno: "")
                    
                    updateOrderStatusISyncAnimalMaster(entity: Entities.animalAddTblEntity, animalTag: scanAnimalText, barCodetag: scanBarcodeText.text!, date: dateVale , damId: damtexfield.text!, sireId: sireIdTextField.text?.uppercased() ?? "" , gender: genderString,update: "true", permanentId: "", tissuName: sampleName, breedName: breedBtnOutlet.titleLabel!.text!, et: etBtn, farmId:farmIdText, orderId: autoD,orderSataus:"false",breedId:breedId,isSync:"false", providerId: pvid,tissuId: sampleID,sireIDAU: "", nationHerdAU: "", userId: userId,udid:timeStampString,animalId: animalId1, selectedBornTypeId: selectedBornTypeId,isSyncServer:false, adhDataServer: false, editFlagSave: true, territoryGeno: "")
                    ...
                    if fetchDataUpdate.count != 0 {
                        updateDataInMasterAnimalTbl(entity: Entities.dataEntryAnimalAddTbl, animalTag: scanAnimalText, barCodetag: scanBarcodeText.text!, date: dateVale , damId: damtexfield.text!.uppercased(), sireId: sireIdTextField.text?.uppercased() ?? "" , gender: genderString,update: "false", permanentId: "", tissuName: sampleName, breedName: breedBtnOutlet.titleLabel!.text!, et: etBtn, farmId:farmIdText, orderId: autoD,orderSataus:"false",breedId:breedId,isSync:"false", providerId: pvid,tissuId: sampleID,sireIDAU: "", nationHerdAU: "", userId: userId,udid:timeStampString,animalId: animalId1, selectedBornTypeId: selectedBornTypeId,isSyncServer:false, adhDataServer: false, custmerId: custmerId , editFlagSave: true)
                ...
                updateOrderStatusISyncAnimalMaster(entity: Entities.animalMasterTblEntity, animalTag: scanAnimalText, barCodetag: scanBarcodeText.text!, date: dateVale , damId: damtexfield.text!.uppercased(), sireId: sireIdTextField.text?.uppercased() ?? "" , gender: genderString,update: "false", permanentId: "", tissuName: sampleName, breedName: breedBtnOutlet.titleLabel!.text!, et: etBtn, farmId:farmIdText, orderId: autoD,orderSataus:"false",breedId:breedId,isSync:"false", providerId: pvid,tissuId: sampleID,sireIDAU: "", nationHerdAU: "", userId: userId,udid:timeStampString,animalId: Int(existAnimalData.animalId), selectedBornTypeId: selectedBornTypeId,isSyncServer:false, adhDataServer: false, editFlagSave: true, territoryGeno: "")
                ...
                if fetchDataUpdate.count != 0 {
                    updateDataInMasterAnimalTbl(entity: Entities.dataEntryAnimalAddTbl, animalTag: scanAnimalText, barCodetag: scanBarcodeText.text!, date: dateVale , damId: damtexfield.text!.uppercased(), sireId: sireIdTextField.text?.uppercased() ?? "" , gender: genderString,update: "false", permanentId: "", tissuName: sampleName, breedName: breedBtnOutlet.titleLabel!.text!, et: etBtn, farmId:farmIdText, orderId: autoD,orderSataus:"false",breedId:breedId,isSync:"false", providerId: pvid,tissuId: sampleID,sireIDAU: "", nationHerdAU: "", userId: userId,udid:timeStampString,animalId: Int(existAnimalData.animalId), selectedBornTypeId: selectedBornTypeId,isSyncServer:false, adhDataServer: false, custmerId: custmerId , editFlagSave: true)
            ...
            barAutoPopu = false
            self.male_femaleBtnOutlet.titleLabel?.textColor = UIColor.black
            byDefaultSetting()
            etBtn.removeAll()
