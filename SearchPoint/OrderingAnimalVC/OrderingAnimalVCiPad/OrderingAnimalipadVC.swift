...
class OrderingAnimalipadVC : UIViewController,VNDocumentCameraViewControllerDelegate {
    
    //MARK: IB OUTLETS
    @IBOutlet weak var mergeListView: UIView!
    @IBOutlet weak var cartView: UIView!
    @IBOutlet weak var sireIDTopView: UIView!
    @IBOutlet weak var sireIDViewAustralia: UIView!
    ...
    @IBOutlet weak var sireIdView: UIView!
    @IBOutlet weak var lblOnFarmIdTitle: UILabel!
    @IBOutlet weak var lblOfficialTagTitle: UILabel!
    ...
    @IBOutlet weak var tissueBtnOutlet: UIButton!
    ...
    @IBOutlet weak var sireTipBTNoUTLET: UIButton!
    ...
    var dateVale = ""
    var attrs : [NSAttributedString.Key: Any] = [.font: UIFont.systemFont(ofSize: 19), .foregroundColor: UIColor.init(red: 57/225, green: 69/225, blue: 73/225, alpha: 1.0)]
  ...
  func NavigateToOrderingProductSelectionScreen(screenType: Int = 1) {
    ...
    func sideMenuRevealSettingsViewController() -> OrderingAnimalipadVC? {
        var viewController: UIViewController? = self
        
        if viewController != nil && viewController is OrderingAnimalipadVC {
            return viewController! as? OrderingAnimalipadVC
        }
        while (!(viewController is OrderingAnimalipadVC) && viewController?.parent != nil) {
            viewController = viewController?.parent
        }
        if viewController is OrderingAnimalipadVC {
            return viewController as? OrderingAnimalipadVC
        }
        return nil
    }
...
//        crossedBtnOutlet.isHidden = true
    ...
    func tableViewpop() {
        ...
        buttonbg2.addTarget(self, action:#selector(OrderingAnimalipadVC.buttonPreddDroper), for: .touchUpInside)
        ...
        droperTableView.layer.borderColor =  UIColor(displayP3Red: 57/255, green: 69/255, blue: 73/255, alpha: 1).cgColor
    ...
    func tableViewpopAutosugesstion() {
        ...
        AutoSuggestionTableView.layer.borderColor =  UIColor(displayP3Red: 57/255, green: 69/255, blue: 73/255, alpha: 1).cgColor
    ...
    func validationId17(animalId:String) {
        ...
        if idAnimal.isInt == true {
            ...
            if firstElement.isInt == true {
                ...
                var fourCHAR = idAnimal.dropFirst(4).uppercased()
                ...
                if second.isInt == true {
                    if thirdCHAR.isInt == true {
                        ...
                        if fourChar.isInt == true {
                            ...
                            var thirdC = breed.dropLast(1).uppercased()
                            ...
                            if breedAr.contains(ab) {
                                ...
                                var alertAnimId = idAnimal.uppercased()
                                let addObject = 12 - idAnimal.count
                                if idAnimal.count <= 12 {
                                    if addObject != 0 {
                                        for _ in 0...addObject - 1{
                    ...
                    if thirdCHAR.isInt == true {
                        var ab = validateFirstLetter(firstLetter: Character(second))
                        var idAnimlFailNaab = animalId.uppercased()
                        if breedAr.contains(ab)  {
                        ...
                        var dropFirst = getThi.dropFirst(1).uppercased()
                        var brFirstLetter = Character(dropFirst.prefix(1).uppercased())
                ...
                if firstThreeElement.isInt != true {
                    if threeCharCode.contains(firstThreeElement) {
                            ...
                            if String(genderGet) == "M" || String(genderGet) == "F"{
                                ...
                                if removeSeven.count <= 12 {
                                    ...
                                    if getCountryCode == "840" || getCountryCode == "USA"{
                                        ...
                                        var breedTwo = (breedC![indexOfA] as? String)!
                        ...
                        if breedAr.contains(firstTwo) || XDFound  {
                            var getTwoBreedCode = firstThreeElement.dropLast(1)
