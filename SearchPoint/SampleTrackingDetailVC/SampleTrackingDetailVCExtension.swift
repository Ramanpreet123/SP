//
//  SampleTrackingDetailVCExtension.swift
//  SearchPoint
//
...
extension SampleTrackingDetailVC:offlineCustomView{
    func crossBtnCall() {
...
extension SampleTrackingDetailVC :UITableViewDelegate,UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return self.samples.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell:SampleTrackingDetailCell = self.tblView.dequeueReusableCell(withIdentifier: "cell") as! SampleTrackingDetailCell
        cell.selectionStyle = .none
        cell.tableInnerview.layer.cornerRadius = 10
        cell.tableInnerview.layer.borderWidth = 1
        tableView.isHidden = false
        let data = samples[indexPath.row]
        
        if filterName == LocalizedStrings.reportedFilterName {
            cell.orderIdBttn.backgroundColor = UIColor(red: 1/255, green: 166/255, blue: 198/255, alpha: 1)
            cell.tableInnerview.layer.borderColor = UIColor(red: 1/255, green: 166/255, blue: 198/255, alpha: 1).cgColor
            ...
            cell.tableInnerview.layer.backgroundColor = UIColor(red: 247/255, green: 1, blue: 245/255, alpha: 1).cgColor
            cell.officialIDLbl.text = data.officialId ?? "NA"
            cell.barcode_Lbl.text = data.sampleBarcode ?? "NA"
            cell.onFarmIdTitleLabel.text = LocalizedStrings.onFarmIdText
            cell.onFarm_IDLbl.text = data.onFarmId ?? "NA"
    ...
    func getProductWithStatus(sampleDetails: [SampleDetails]) -> NSMutableAttributedString {
        ...
        for detail in sampleDetails {
            let productTitle = NSMutableAttributedString(string:NSLocalizedString(LocalizedStrings.productText, comment: "")  , attributes: [NSAttributedString.Key.font: UIFont.systemFont(ofSize: 12), NSAttributedString.Key.foregroundColor: titleColor])
            let productNameValue = NSMutableAttributedString(string: ": \((detail.productName ?? "")) \n", attributes: [NSAttributedString.Key.font: UIFont.systemFont(ofSize: 12), NSAttributedString.Key.foregroundColor: UIColor.black])
            productTitle.append(productNameValue)
            
            if detail.actionRequired?.isEmpty == false {
                let actionRequiredTitle = NSMutableAttributedString(string:NSLocalizedString(ButtonTitles.actionRequiredText, comment: "") , attributes: [NSAttributedString.Key.font: UIFont.systemFont(ofSize: 12), NSAttributedString.Key.foregroundColor: titleColor])
            ...
            let statusTitle = NSMutableAttributedString(string: NSLocalizedString("Status", comment: ""), attributes: [NSAttributedString.Key.font: UIFont.systemFont(ofSize: 12), NSAttributedString.Key.foregroundColor: titleColor])
...
extension SampleTrackingDetailVC : SideMenuUI{
