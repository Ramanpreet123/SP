//
//  ADHAnimalVCExtensionIpad.swift
//  SearchPoint
//
...
extension ADHAnimalVCIpad :filterInfoUpdate{
...
extension ADHAnimalVCIpad:UITableViewDelegate,UITableViewDataSource {
...
//        return cell
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if searchFound{
            if(self.viewModel.filterAdhAnimalData[indexPath.row].animalbarCodeTag == "" ||  self.viewModel.filterAdhAnimalData[indexPath.row].animalbarCodeTag == nil) {
                for (index, _) in viewModel.filterAdhAnimalData.enumerated() {
                    if(index == indexPath.row){
                        self.viewModel.filterAdhAnimalData[indexPath.row].showTextField.toggle()
                    }
                    else{
                        self.viewModel.filterAdhAnimalData[index].showTextField = false
                    }
                }
            }
            else {
                let pvID =  UserDefaults.standard.integer(forKey:keyValue.providerID.rawValue)
                updateADhAnimalBarcode(entity: Entities.animalMasterTblEntity, officialID: self.viewModel.filterAdhAnimalData[indexPath.row].animalTag!, barCode: self.viewModel.filterAdhAnimalData[indexPath.row].animalbarCodeTag ?? "", pvID: pvID , isADHSelected: !(self.viewModel.filterAdhAnimalData[indexPath.row].isADHSelected))
                showTotalCount()
            }
        }
        else {
            if(self.viewModel.adhAnimalData[indexPath.row].animalbarCodeTag == "" ||  self.viewModel.adhAnimalData[indexPath.row].animalbarCodeTag == nil) {
                
                for (index, _) in viewModel.adhAnimalData.enumerated() {
                    if(index == indexPath.row){
                        self.viewModel.adhAnimalData[indexPath.row].showTextField.toggle()
                    }
                    else{
                        self.viewModel.adhAnimalData[index].showTextField = false
                    }
                }
            }
            else {
                let pvID =  UserDefaults.standard.integer(forKey:keyValue.providerID.rawValue)
                updateADhAnimalBarcode(entity: Entities.animalMasterTblEntity, officialID: self.viewModel.adhAnimalData[indexPath.row].animalTag!, barCode: self.viewModel.adhAnimalData[indexPath.row].animalbarCodeTag ?? "", pvID: pvID , isADHSelected: !(self.viewModel.adhAnimalData[indexPath.row].isADHSelected))
                showTotalCount()
            }
        }
        self.tblView.reloadData()
...
extension ADHAnimalVCIpad : UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell: ADHListCollectionViewCell = self.adhCollectionView.dequeueReusableCell(withReuseIdentifier: "cell", for: indexPath) as! ADHListCollectionViewCell
     //   cell.scanBarCodeTextField.delegate = cell
        if searchFound {
            cell.data = self.viewModel.filterAdhAnimalData[indexPath.row]
            guard let animalData = self.viewModel.filterAdhAnimalData[indexPath.row] as? AnimalMaster else {
        ...
        cell.addBarcodeAction = { [unowned self] ( error) in
            
            let animalData = self.viewModel.adhAnimalData[indexPath.row] as? AnimalMaster
            self.setAnimalData(animalData: animalData!, index: indexPath.row)
            if searchFound{
                if(self.viewModel.filterAdhAnimalData[indexPath.row].animalbarCodeTag == "" ||  self.viewModel.filterAdhAnimalData[indexPath.row].animalbarCodeTag == nil) {
                    for (index, _) in viewModel.filterAdhAnimalData.enumerated() {
//                        if(index == indexPath.row){
...
//                        }
                        ...
                        self.showBarcodeView(isHidden: false, animalData: animalData)
                    }
                }
                else {
                    let pvID =  UserDefaults.standard.integer(forKey:keyValue.providerID.rawValue)
...
//    }
    ...
    func setAnimalData(animalData: AnimalMaster, index: Int) {
        scanBarCodeTextField.tag = index
        scanBarCodeButton.tag = index
...
//        scanBarCodeTextField.addTarget(self, action: #selector(editingDidBegin), for: .editingDidBegin)
    ...
    @objc func acnScanButton(sender: UIButton) {
        
        if let cell = adhCollectionView.cellForItem(at: IndexPath(row: sender.tag, section: 0)) as? ADHListCollectionViewCell{
            self.openCamForBarCodeScan(cell)
            barcodeScreen = true
        }
    }
    
    func showBarcodeView(isHidden : Bool?, animalData: AnimalMaster?) {
        self.barcodeView.isHidden = isHidden!
        self.calendarViewBkg.isHidden = isHidden!
        
        if let animalData = animalData {
            farmIdLbl.text = animalData.farmId
        }
    }
}

//MARK: OFFLINE CUSTOMVIEW EXTENSION
extension ADHAnimalVCIpad:offlineCustomView{
    func crossBtnCall() {
...
extension ADHAnimalVCIpad : SideMenuUI,RFID,nearByDevice{
...
extension ADHAnimalVCIpad: ADHFilterProtocol {
    func openCamForBarCodeScan(_ cell: ADHListCollectionViewCell) {
        if let indexPath = adhCollectionView?.indexPath(for: cell)  {
...
//    func textField(editingChangedInTextField newText: String, in cell: ADHListCell) {}
    ...
    func textFieldEndedEditing(finalText barCode: String, in cell: ADHListCollectionViewCell) {
        if searchFound {
            if let indexPath = adhCollectionView?.indexPath(for: cell) {
            ...
            if let indexPath = adhCollectionView?.indexPath(for: cell) {
                ...
                showTotalCount()
                DispatchQueue.main.async {
                    self.scanBarCodeTextField.text = ""
                    self.adhCollectionView.reloadData()
                }
            }
        }
...
extension ADHAnimalVCIpad: QrScannerProtocol {
    func qrCodeScannedResult(_ qrValue: String) {
        guard let cell  = adhCollectionView.cellForItem(at: cellIndexForScanner!) as? ADHListCollectionViewCell else {
            return
        }
        ...
        if searchFound {
            ...
            if (adhCollectionView?.indexPath(for: cell)) != nil {
                self.scanBarCodeTextField.text = qrValue
...
extension ADHAnimalVCIpad: scannedOCRProtocol {
    func ocrDetected(_ scannedResult: String) {}
}

//MARK: FETCH AND CREATE DATA LIST
private typealias DataListCartHelper = ADHAnimalVCIpad
...
extension ADHAnimalVCIpad : UITextFieldDelegate {
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        guard !string.isEmpty else {
            self.searchForADHAnimal(seachText: string)
            return true
        }
    ...
    //    return true
    //}
//        @objc func editingDidBegin() { self.textField(editingDidBeginIn: ) }
//        @objc func textFieldValueChanged(_ sender: UITextField) {
//            if let text = sender.text { self.textField(editingChangedInTextField: text, in: self) }    }
        @objc func editingEnded(_ sender: UITextField) {
            if let text = sender.text {
                if let cell = adhCollectionView.cellForItem(at: IndexPath(row: sender.tag, section: 0)) as? ADHListCollectionViewCell{
                    self.barcodeText = text
                    self.adhCollectionViewCell = cell
...
extension ADHAnimalVCIpad : objectPickCartScreen {
    func objectGetOnSelection(temp: Int) {}
}

//MARK: IMAGE PICKER CONTROLLER DELEGATE
extension ADHAnimalVCIpad : UIImagePickerControllerDelegate & UINavigationControllerDelegate {
...
//extension ADHAnimalVCIpad : ADHBarcodeView {
