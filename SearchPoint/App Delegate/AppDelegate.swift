...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        if UserDefaults.standard.value(forKey: keyValue.firstLogin.rawValue) == nil{
            _ = self.keychainDeleteObjectForKey("userName")
            _ = self.keychainDeleteObjectForKey("emailId")
            _ = self.keychainDeleteObjectForKey("password")
        ...
        if !UserDefaults.standard.bool(forKey:"Provider") {
    ...
    func applicationWillEnterForeground(_ application: UIApplication) {
        ...
        let maximumTime = 55
        if mintues >= maximumTime && mintues < 60 && Connectivity.isConnectedToInternet() {
    ...
    func callGetSpecies(){
        guard let accessToken = UserDefaults.standard.value(forKey: "accessToken") as? String, !accessToken.isEmpty else {
        ...
        network.CallApi(packet: NetwrokPackets(
            Method: HTTPMethod.get,
            Headers: headerDict,
            bodyParams: nil,
            Url: Configuration.Dev(packet: ApiKeys.getSpecies.rawValue).getUrl()
        ))
    }
    ...
    func getDeviceIdentifierFromKeychain() -> String {
        ...
        var deviceUDID = self.keychainValueForKey("keychainDeviceUDID") as? String
        if deviceUDID == nil {
            deviceUDID = UIDevice.current.identifierForVendor!.uuidString
            self.keychainSetObject(deviceUDID! as AnyObject, forKey: "keychainDeviceUDID")
        }
        return deviceUDID!
    }
    ...
    func keychainSetObject(_ object: AnyObject, forKey: String) {
    ...
    func keychainDeleteObjectForKey(_ key: String) -> Bool {
        let result = JNKeychain.deleteValue(forKey: key)
        return result
    }
    
    func keychainValueForKey(_ key: String) -> AnyObject? {
