//
//  OPSSecondVCActionMethods.swift
//  SearchPoint
//
        ...
        if  UserDefaults.standard.object(forKey:  keyValue.fOSampleTrackingDetailVC.rawValue) as! String == ButtonTitles.earTagText{
            ...
            if self.animaId == 0{
                ...
                if pvid == 3 && UserDefaults.standard.value(forKey: keyValue.isAuSelected.rawValue) as? String == keyValue.noNeedAuPopUp.rawValue {
                    ...
                    self.fethData =  fetchAllDataanimalTag(entityName: Entities.productAdonAnimalTblEntity,asending : ascendingFound,orderId:self.orderId,userId:self.userId, animalTag: self.serchTextField.text!)
                }
            ...
            if self.barCodeId == 0{
                ...
                if pvid == 3 && UserDefaults.standard.value(forKey: keyValue.isAuSelected.rawValue) as? String == keyValue.noNeedAuPopUp.rawValue {
                    ...
                    self.fethData = fetchAllDataBarcOde(entityName: Entities.productAdonAnimalTblEntity, asending: ascendingFound,orderId:self.orderId,userId:self.userId, barcode: self.serchTextField.text!)
                }
                self.fetchProductAdonAnimalTbl(fethData: self.fethData, completion: {})
                self.barCodeId = 1
            }
