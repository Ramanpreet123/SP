...
extension OPSSecondVCiPad:offlineCustomView{
    func crossBtnCall() {
...
extension OPSSecondVCiPad : SideMenuUI{
...
private typealias DeleteAnimalListHelper = OPSSecondVCiPad
...
extension OPSSecondVCiPad : UITableViewDelegate, UITableViewDataSource {
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        if let item = data[keyValue.animalId.rawValue] as? Int{
            if values[indexPath.row] == item{
                ...
                if data!.count > 0 {
                  //  self.tblViewHeightConstraint.constant = 590
                    if data!.count % 2 == 0{
           ...
           // self.tblViewHeightConstraint.constant = 410
        }
        var height:CGFloat
        var bCount = (arr1[values[indexPath.row]]!.count)
        if bCount % 2 == 0{
            if pviduser != 4 {
                if pviduser == 11 {
                    height = 40 * CGFloat(bCount/2) + 180 + CGFloat((bCount/2)*20) + CGFloat(addon + 100)
                }
                
                else if pviduser == 3 {
                    height = 40 * CGFloat(bCount/2) + 180 + CGFloat((bCount/2)*20) + CGFloat(addon + 70)
                }
                
                else if pviduser == 8 {
                    height = 40 * CGFloat(bCount/2) + 180 + CGFloat((bCount/2)*20) + CGFloat(addon + 110)
                }
                ...
                height = 40 * CGFloat(bCount/2) + 180 + CGFloat((bCount/2)*20) + CGFloat(addon + 120)
            }
            
        }
        else {
            height = 40 * CGFloat((bCount/2)+1) + 180 + CGFloat(((bCount/2)+1)*20) + CGFloat(addon + 70)
            if pviduser != 4 {
                ...
                if pviduser == 11 {
                    ...
                    height = 40 * CGFloat((bCount/2)+1) + 180 + CGFloat(((bCount/2)+1)*20) + CGFloat(addon + 80)

                }
            } else {
                height = 40 * CGFloat((bCount/2)+1) + 180 + CGFloat(((bCount/2)+1)*20) + CGFloat(addon + 120)
            }
        }
        return height
    }
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        else if tableView == billingTableView {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if tableView == tbblView {
            ...
            if datavale.farmId != ""{
                ...
                cell1.firstAnimalTag?.text = "N/A" // Or any default placeholder
            }
            if datavale.animalTag != ""{
                cell1.firstBarcode?.text = datavale.animalTag
            } else {
                cell1.firstBarcode?.text = "N/A" // Or any default placeholder
            }
//            cell1.firstAnimalTag?.text = datavale.farmId
//            cell1.firstBarcode?.text = datavale.animalTag
...
//                return cell
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if tableView == tbblView {
            ...
            self.customerNameLbl.text = farmAddr[indexPath.row].contactName
...
extension OPSSecondVCiPad : UICollectionViewDataSource,UICollectionViewDelegate,UICollectionViewDelegateFlowLayout {
        ...
        if indexPath.section == 0 {
            if indexPath.row == 0 {
               ...
               return item
                ...
                let productName = arr1[values[collectionView.tag]]![indexPath.row - 2].productName!
                item.infoBtnOutlet.isHidden = true
                var data = GetProductTbl()
                var breedId =  ""
                let animalData = fetchAnimaldataBarcodeanimalId(entityName: Entities.animalAddTblEntity, animalbarCodeTag: arr1[values[collectionView.tag]]![indexPath.row - 2].animalbarCodeTag ?? "",  animalId: Int(arr1[values[collectionView.tag]]![indexPath.row - 2].animalId), orderStatus: "false", custId: custmerId) as! [AnimaladdTbl]
                ...
                if productArr.count > 0 {
                    data = productArr[indexPath.item - 2] as! GetProductTbl
                }
           
            //    item.Lbl.layer.borderColor = UIColor.lightGray.cgColor
      
                if UserDefaults.standard.bool(forKey: keyValue.bvdvSelected.rawValue){
                    let data12333 =  fetchProductAdonDataBVDV(entityName: Entities.subProductTblEntity, adonId:LocalizedStrings.bvdvAddOnId,ordrId:orderId, productId: Int(arr1[values[collectionView.tag]]![indexPath.row - 2].productId))
                    
                    if data12333.count == 0{
                        updateProductTablaid(entity: Entities.productAdonAnimalTblEntity, productId: Int(arr1[values[collectionView.tag]]![indexPath.row - 2].productId), status: "false")
                    }
                }
                if arr1[values[collectionView.tag]]![indexPath.row - 2].status! == "true"{
                  ...
                  //  item.Lbl.textColor = UIColor.black
                    item.Lbl.textColor = UIColor.black
                    item.Lbl.layer.borderColor = UIColor(displayP3Red: 191/255, green: 202/255, blue: 205/255, alpha: 1).cgColor
                }
...
//                }
                  ...
                  //  self?.lable2TopConstraint.constant = 8
                    ...
                    if data.providerId ==  2{
                        if data.productName == LocalizedStrings.providerClarifidePlusUpgrade {
                            ...
                            let helloAttributes1: [NSAttributedString.Key: Any] = [NSAttributedString.Key.font: UIFont.systemFont(ofSize: 16.0, weight: .regular)]
                            let worldAttributes1: [NSAttributedString.Key: Any] = [NSAttributedString.Key.font: UIFont.systemFont(ofSize: 16.0, weight: .regular)]
                            
                            let helloString1 = NSAttributedString(string: LocalizedStrings.zoetisSupportedBreeds.localized, attributes: helloAttributes1)
                           ...
                           // worldString1.append(helloString1)
    ...
    //    return CGSize(width: (collectionView.frame.size.width / 2) - 15, height: self.view.frame.height / 15)
        if indexPath.section == 0 {
            if indexPath.row == 0 {
                ...
                return CGSize(width: (collectionView.frame.width/4) - 6, height: 60)
            }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if indexPath.section == 0 {
            ...
            let PName =  arr1[values[collectionView.tag]]![indexPath.row - 2].productName
            aTag = Int(arr1[values[collectionView.tag]]![indexPath.row - 2].animalId)
            let productname = arr1[values[collectionView.tag]]![indexPath.row - 2].animalTag
            let farmId = arr1[values[collectionView.tag]]![indexPath.row - 2].farmId
