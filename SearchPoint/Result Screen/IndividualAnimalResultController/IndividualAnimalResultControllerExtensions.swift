//
//  IndividualAnimalResultControllerExtensions.swift
//  SearchPoint
//
...
extension IndividualAnimalResultController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
                ...
                case let .success(value):
                    ...
                    if groupid?.value(forKey: keyValue.messageKey.rawValue) as! String == LocalizedStrings.noPhotoSaved{
                        ...
                        if imageBase64 != "" {
                            ...
                            let img =   newImage
                            if UserDefaults.standard.value(forKey: keyValue.groupDetails.rawValue) as? String == keyValue.groupName.rawValue{
                                updategroupPhotoAgainstFarmIdOfficalId(entity: Entities.resultGroupAnimalsTblEntity,custmerId: Int(self.custmerID ),onFarmID: self.onFarmIdLbl.text ?? "",officialID: self.officialTagLbl.text ?? "",photos: img)
                            }
                            else{
                                updatePhotoAgainstFarmIdOfficalId(entity: Entities.resultMyherdDataTblEntity,custmerId: Int(self.custmerID ),onFarmID: self.onFarmIdLbl.text ?? "",officialID: self.officialTagLbl.text ?? "",photos: img)
                            }
                        }
                    }
        ...
        self.dismiss(animated: true) { [weak self] in
            ...
            if Connectivity.isConnectedToInternet() {
                ...
                self?.photoupdateinapi(animalId: self?.animalID ?? "", photoBase64Encoded: binaryString ?? "", fileName: "", completionHandler: { (success) -> Void in
                    print(success)
                    updatePhotoAgainstFarmIdOfficalId(entity: Entities.resultMyherdDataTblEntity,custmerId: Int(self?.custmerID ?? 0),onFarmID: self?.onFarmIdLbl.text ?? "",officialID: self?.officialTagLbl.text ?? "",photos: image)
                    self?.hideIndicator()
                })
            }
        }
    ...
    @objc func imageWasSavedSuccessfully(_ image: UIImage, didFinishSavingWithError error: NSError!, context: UnsafeMutableRawPointer){
        if error != nil {
            ...
            if img != nil{
                ...
                let image = UIImage(data: imageData)!
                if UserDefaults.standard.value(forKey: keyValue.groupDetails.rawValue) as? String == keyValue.groupName.rawValue{
                    updategroupPhotoAgainstFarmIdOfficalId(entity: Entities.resultGroupAnimalsTblEntity,custmerId: Int(self.custmerID ),onFarmID: self.onFarmIdLbl.text ?? "",officialID: self.officialTagLbl.text ?? "",photos: image)
                }
                else{
                    updatePhotoAgainstFarmIdOfficalId(entity: Entities.resultMyherdDataTblEntity,custmerId: Int(self.custmerID ),onFarmID: self.onFarmIdLbl.text ?? "",officialID: self.officialTagLbl.text ?? "",photos: image)
                }
            }
        }
    }
