...
class Resulttotalanimalupdate{
    ...
    func callTotalanimalsApi()
    {
        ...
        print(activeCustomerId ?? 0)
        Network.CallApi(packet: NetwrokPackets( Method: HTTPMethod.get, Headers: headerDict, bodyParams: nil, Url:Configuration.Dev(packet: ApiKeys.resulttotalanimals.rawValue + "/\(activeCustomerId ?? 0)" + "/\(currentdate)").getUrl()))
    }
