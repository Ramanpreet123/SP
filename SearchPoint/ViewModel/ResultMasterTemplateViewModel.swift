                ...
                for traidloopname in traitNamesArr! {
                    ...
                    for (index, element) in traitheader!.enumerated()
                    {
                          ...
                          if headerOfId ==  header && indexIds == indextid
                        {
                            ...
                            {
                                saveTraitHeaderMaster(entity: Entities.resultTraitHeaderTblEntity,productName: productname,species: species ,headerName:header!.capitalized ,headerId: name ?? "",traidid: traidid)
                            }
                       }
...
//
//
////                        saveTraitHeaderMaster(entity: Entities.resultTraitHeaderTblEntity,productName: result.name ?? "",species: result.species ?? "" ,headerName:header.header.capitalized ,headerId: headIds. ?? "")
//
//                    }
//
//                }
...
//
//
//                    saveMasterTemplate(entity: Entities.resultMasterTemplateTblEntity, notes: headerTemp.notes , attributeName: headerTemp.attribute , calving: headerTemp.calving , defaultSortOrder: headerTemp.defaultSortOrder , displayName : headerTemp.displayName ?? "", fertility : headerTemp.fertility ?? "", geneticConditions : headerTemp.geneticConditions ?? "", indexes: headerTemp.indexes ?? "", location : headerTemp.location ?? "", milkProteins: headerTemp.milkProteins ?? "", parentage: headerTemp.parentage ?? "", photos: headerTemp.photos ?? "" , productName: result.name ?? "", profile: headerTemp.profile ?? "", results: headerTemp.results ?? 0, sortBy: headerTemp.sortBy , speciesName: result.species ?? "", traitId: headerTemp.traitID ?? "", type: headerTemp.type ?? "", wellness: headerTemp.wellness ?? "")
//
//                }
//
//            }
