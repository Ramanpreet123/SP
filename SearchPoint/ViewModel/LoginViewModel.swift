...
class  LoginViewModel{
    ...
    private func getParams()->[String:Any]{
        var email = dependency?.emailTextField.text! ?? ""
        var password = dependency?.pwdTextField.text! ?? ""
       ...
       // "password":password ,
        let decrypt = aes256?.decrypt(data: encryptedPassword256)
        return ["appId":"\(appID)",
            "email":email,
    ...
    func saveLoginDat(dataModel:LoginModel){
      ...
      UserDefaults.standard.synchronize()
        ...
        appDelegate?.keychain_setObject(dependency?.emailTextField.text! as AnyObject, forKey: "userName")
        appDelegate?.keychain_setObject(dependency?.pwdTextField.text! as AnyObject, forKey: "password")
        ...
        let saveObject : [String:Any] = ["agreedToTermsOfService":dataModel.agreedToTermsOfService , "errorDetail":dataModel.errorDetail , "marketCode":dataModel.marketCode,"message":"","authorizationToken":dataModel.authorizationToken,keyValue.userId.rawValue:"1","mustValidateDevice":dataModel.mustValidateDevice,"alert":dataModel.alert]
...
extension LoginViewModel:ResponseDelegate{
    func responseRecieved(_ data: Data?, status: Bool) {
        if status == false {
            ...
            if modalObject?.errorDetail == "Invalid Credentials"  || modalObject?.errorDetail ==  "INVALID_CREDENTIALS" || modalObject?.authorizationToken == "" || modalObject?.authorizationToken == nil{
                delegate1?.responseRecievedStatusForLoginDevice(status: status, response: modalObject)
                return
            }
            else if  modalObject?.errorDetail == "invalid loginID or password" || modalObject?.errorDetail == "INVALID LOGINID OR PASSWORD" || modalObject?.authorizationToken == ""{
                delegate1?.responseRecievedStatusForLoginDevice(status: status, response: modalObject)
                return
            }
