...
class ResultFilterViewModel
{
    ...
    func callSaveListApi() {
       ...
       // let accessToken = appDelegate?.keychain_valueForKey(keyValue.accessToken.rawValue) as? String
        ...
        Network.CallApi(packet: NetwrokPackets( Method: HTTPMethod.get, Headers: headerDict, bodyParams: nil, Url:Configuration.Dev(packet: ApiKeys.resultfilter.rawValue + "/\(currentCustomerId ?? 0)").getUrl()))
    }
