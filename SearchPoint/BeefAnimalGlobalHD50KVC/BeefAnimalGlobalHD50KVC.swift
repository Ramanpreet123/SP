...
class BeefAnimalGlobalHD50KVC : UIViewController,UIImagePickerControllerDelegate & UINavigationControllerDelegate,VNDocumentCameraViewControllerDelegate {
    ...
    func NavigateToBeefOrderingScreen() {
        ...
        let dataListAnimals : [BeefAnimaladdTbl] = viewAnimalArray 
    ...
    func addBtnCondtion(completionHandler: CompletionHandler){
        ...
        if naabFetch1!.count != 0 {
            let fetchedTissueId = naabFetch1![0] as? Int
            self.tissuId = fetchedTissueId!
        }
        ...
        if animalData.count > 0  {
            ...
            let productId = product[0] as! GetProductTblBeef
            ...
            if data12333.count > 0{
                if tissueBttnOutlet.titleLabel!.text! == ButtonTitles.allflexTSUText || tissueBttnOutlet.titleLabel!.text! == ButtonTitles.allflexTSTText || tissueBttnOutlet.titleLabel!.text! == ButtonTitles.caisleyTSUText {
                    ...
                    let cancelAction = UIAlertAction(title: NSLocalizedString("NO", comment: ""), style: UIAlertAction.Style.default) {
                        ...
                        let animalCount =  fetchAllDataAnimalDatarderId(entityName: Entities.beefAnimalAddTblEntity, userId: self.userId,orderId:orderId,orderStatus:"false", providerId: UserDefaults.standard.integer(forKey: keyValue.beefPvid.rawValue))
    ...
    private func setVisionTextRecognizeImage(image: UIImage) {
        ...
        request = VNRecognizeTextRequest { [weak self] request, error in
            ...
            DispatchQueue.main.async {
                self.handleRecognizedText(textStr)
            }
        }
    ...
    private func handleRecognizedText(_ text: String) {
        let trimmed = String(text.filter { !" \n\t\r(),.-[]:}{/".contains($0) })
        let upperText = trimmed.uppercased()
        methReturn = ukTagReutn(animalId: upperText)
        
        let isGlobalHD = UserDefaults.standard.string(forKey: keyValue.beefProduct.rawValue) == keyValue.globalHD50K.rawValue
        
        if methReturn == LocalizedStrings.againClick {
            showScanRetryAlert(for: upperText, isGlobalHD: isGlobalHD)
            if isGlobalHD {
                scanEarTagTextField.text = ""
            } else {
            ...
            if isGlobalHD {
                scanEarTagTextField.text = methReturn
                dataPopulateInFocusChange()
                textFieldBackroungWhite()
            } else {
                inheritEarTagTextfield.text = methReturn
                inherittextFieldBackroungWhite()
                dataPopulateInFocusChange()
            }
        }
    ...
    private func showScanRetryAlert(for scannedValue: String, isGlobalHD: Bool) {
        let message = LocalizedStrings.unableToReadValue.localized(with: scannedValue)
        ...
        alert.addAction(UIAlertAction(title: NSLocalizedString(LocalizedStrings.useScannedValue, comment: ""), style: .default) { _ in
            if isGlobalHD {
                self.scanEarTagTextField.text = scannedValue
                self.textFieldBackroungWhite()
            } else {
                self.inheritEarTagTextfield.text = scannedValue
    ...
    func ukTagReutn(animalId:String)-> String{
        let animalID = animalId.uppercased()
        ...
        if stringResult  {
            ...
            let stringResultUS = animalID.contains("US")
            let stringResult840 = animalID.contains("840")
