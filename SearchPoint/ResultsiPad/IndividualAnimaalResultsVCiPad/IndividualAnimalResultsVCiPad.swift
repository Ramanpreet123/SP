...
//        }
        ...
        initialNetworkCheck()
        if navigateFromGroupDetailScreen == true {
        ...
        if UserDefaults.standard.value(forKey: "groupdetails") as? String == "groupName"
        {
          ...
          if  animal.count > 0 {
            ...
            if dollar.count > 0 {
              ...
              let groupTypeId = gruupCreate?.groupTypeId  ?? 0
              ...
              let gTypeId = gruupCreate?.groupTypeId  ?? 0
              ...
              if ban.count > 0 {
                ...
                let gtId = gruupCreate?.groupTypeId
                let gTypeName = gruupCreate?.groupTypeName
                ...
                removeAnimalInGroup(groupId: serverGid , animalIds: animalID ?? "", completionHandler: { (result) in
                  if result == true{
                    deletGroupInfoResultGroupDetail(entityName: "ResultGroupsAnimals", groupName: gname ?? "", customerId: self.custmerID, onFarmId: onFarmid ?? "", officalId: officialID ?? "")
                    ...
                    else
                    {
                      ...
                      if checkanimal11.count > 0
                      {
                        ...
                        self.removeAnimalInGroup(groupId: servergroupidcheck , animalIds: animalid ?? "", completionHandler: { (result) in
                          if result == true{
                            deletGroupInfoResultGroupDetail(entityName: "ResultGroupsAnimals", groupName: groupnamecheck, customerId: self.custmerID, onFarmId: onFarmid ?? "", officalId: officialID ?? "")
                            let dateFormatter = DateFormatter()
...
//                    cell.notAssignedBtn.setImage(UIImage(named: "radioBtn"), for: .normal)
               ...
               //if ban group not exist add in dollar group
