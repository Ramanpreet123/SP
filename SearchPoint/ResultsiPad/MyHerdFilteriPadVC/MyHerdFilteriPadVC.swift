...
// MARK: - CLASS
class MyHerdFilteriPadVC: UIViewController {
    
    // MARK: - OUTLETS
    
    @IBOutlet weak var breedMainViewHeight: NSLayoutConstraint!
    @IBOutlet weak var CancleBtnFilterScreenOutlet: UIButton!
    @IBOutlet weak var clearFilterBtnOutlet: UIButton!
    @IBOutlet weak var doneBtnFilterScreenOutlet: UIButton!
    ...
    @IBOutlet weak var maxDobValueLbl: PaddingLabel!
    @IBOutlet weak var minDobValueLbl: PaddingLabel!
    @IBOutlet weak var totalAnimalValueLbl: PaddingLabel!
    ...
    var isHerditySelected: Bool = false {
        didSet {
            ...
            if isHerditySelected {
                yesBtnOutlet.backgroundColor = UIColor(red: 57/255, green: 69/255, blue: 73/255, alpha:1.0)
                noBtnOutlet.backgroundColor = UIColor.white
                yesBtnOutlet.layer.borderColor = UIColor.clear.cgColor
                noBtnOutlet.layer.borderColor = UIColor(red: 57/255, green: 69/255, blue: 73/255, alpha:1.0).cgColor
                yesBtnOutlet.setTitleColor(.white, for: .normal)
                noBtnOutlet.setTitleColor(.black, for: .normal)
            } else {
                noBtnOutlet.backgroundColor = UIColor(red: 57/255, green: 69/255, blue: 73/255, alpha:1.0)
                noBtnOutlet.layer.borderColor = UIColor.clear.cgColor
                yesBtnOutlet.layer.borderColor = UIColor(red: 57/255, green: 69/255, blue: 73/255, alpha:1.0).cgColor
                yesBtnOutlet.backgroundColor = UIColor.white
                noBtnOutlet.setTitleColor(.white, for: .normal)
                yesBtnOutlet.setTitleColor(.black, for: .normal)
            }
    ...
    var saveResulyByPageViewModel: ResulyByPageViewModeliPad?
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        providerCollectionView.register(MyHerdCollectionCelliPad.self, forCellWithReuseIdentifier: "MyHerdCollectionCelliPad")
        breedCollectionvIEW.register(MyHerdCollectionCelliPad.self, forCellWithReuseIdentifier: "MyHerdCollectionCelliPad")
    }
    ...
    func setUIOnDidLoad(){
        if UserDefaults.standard.value(forKey: keyValue.isHerditySelected.rawValue) as? Bool == true {
            yesBtnOutlet.backgroundColor = UIColor(red: 57/255, green: 69/255, blue: 73/255, alpha:1.0)
            noBtnOutlet.backgroundColor = UIColor.white
            yesBtnOutlet.layer.borderColor = UIColor.clear.cgColor
            noBtnOutlet.layer.borderColor = UIColor(red: 57/255, green: 69/255, blue: 73/255, alpha:1.0).cgColor
        ...
        self.minDobValueLbl.layoutIfNeeded()
        self.minDobValueLbl.layer.masksToBounds = true
        self.minDobValueLbl.edgeInset = UIEdgeInsets(top: 10, left: 15, bottom: 10, right: 0)
        self.maxDobValueLbl.layoutIfNeeded()
        self.maxDobValueLbl.layer.masksToBounds = true
        self.maxDobValueLbl.edgeInset = UIEdgeInsets(top: 10, left: 15, bottom: 10, right: 0)
        self.totalAnimalValueLbl.layoutIfNeeded()
        self.totalAnimalValueLbl.layer.masksToBounds = true
        self.totalAnimalValueLbl.edgeInset = UIEdgeInsets(top: 10, left: 15, bottom: 10, right: 10)
        ...
        totalAnimalValueLbl.text = "Total Animals:" + " " + "\(totalanimal)"
        if let minDOB = UserDefaults.standard.value(forKey: keyValue.filterMinDOB.rawValue) as? String, let maxDOB = UserDefaults.standard.value(forKey: keyValue.filterMaxDOB.rawValue) as? String {
            minDobValueLbl.text = (minDOB == "" || minDOB.contains("0001-01-01T")) ? "NA" : "Min DOB:" + " " +  getOrderDate(dateStr: maxDOB)
            maxDobValueLbl.text = (maxDOB == "" || maxDOB.contains("0001-01-01T")) ? "NA" : "Max DOB:" + " " + getOrderDate(dateStr: minDOB)
