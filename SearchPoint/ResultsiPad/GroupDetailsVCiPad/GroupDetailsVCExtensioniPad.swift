//
//  GroupDetailsVCExtensioniPad.swift
//  SearchPoint
//
...
extension GroupDeetailsVCiPad: UITableViewDelegate,UITableViewDataSource, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell : MyHerdCollectionViewCelliPad = self.groupsCollectionView.dequeueReusableCell(withReuseIdentifier: "MyHerdCollectionViewCelliPad", for: indexPath as IndexPath) as! MyHerdCollectionViewCelliPad
        
      //  let cell: MyHerdTblCell = self.tblView.dequeueReusableCell(withIdentifier: "MyHerdTblCell", for: indexPath) as! MyHerdTblCell
        cell.isDetailScreen = true
        ...
        cell.cellGroupView.layer.cornerRadius = 15
        cell.cellGroupView.layer.cornerRadius = 15
        cell.cellGroupView.clipsToBounds = true
        cell.cellGroupView.layer.shadowColor = UIColor.black.withAlphaComponent(0.4).cgColor
        cell.cellGroupView.layer.shadowOffset = CGSize(width: 0, height: 1)
        cell.cellGroupView.layer.shadowOpacity = 0.7
        cell.cellGroupView.layer.shadowRadius = 6
        cell.cellGroupView.layer.masksToBounds = false
       ...
       //     cell.tableviewstackview.distribution = .fillEqually
            cell.idLabel.text = LocalizedStrings.onFarmIdText.localized
        ...
        if self.myHerdArray.count == datasource.count{
            ...
            cell.checksatus = checksatus
            let typecheck = myArray.groupTypeId
            cell.groupTypeId = Int64(typecheck)
            if checksatus == 0{
                cell.groupView.layer.backgroundColor  = UIColor(red: 246/255, green: 92/255, blue: 0/255, alpha:1.0).cgColor
                cell.groupImage.image = UIImage(named: "resultsNAImage")!
                cell.cellGroupImage.image = UIImage(named: "orangeResultsNA")
                ...
                if typecheck == 0{
                    ...
                    cell.cellGroupImage.image = UIImage(named: "dollar_orange")
                    ...
                    cell.cellGroupImage.image = UIImage(named: "barnImageOrange")
        ...
        cell.dollarAction = { [unowned self] ( error) in
          ...
          //  datasource.remove(at: indexPath.row)
            let array = self.myHerdArray[indexPath.row]
        ...
        cell.unAssignedAction = { [unowned self] ( error) in
           ...
           // datasource.remove(at: indexPath.row)
            ...
            if  dataObject.groupTypeName != LocalizedStrings.barnGroupType {
                if dataObject.groupStatus == LocalizedStrings.capsInactiveStatus || dataObject.groupStatus == "Unknown"{
                    CommonClass.showAlertMessage(self, titleStr: NSLocalizedString("", comment: ""), messageStr: NSLocalizedString(LocalizedStrings.noActiveDoller, comment: "") )
                    return
                } else {
                    let dollar =  fetchGroupIdFethSatusG(entityName: Entities.resultGroupCreateTblEntity, customerId: self.customerId ?? 0, groupStatusId: 1, groupTypeId: 0)
                    if dollar.count == 0 {
                        ...
                        let groupServerIdmain = dataObject.serverGroupId ?? ""
                        showIndicator(self.view, withTitle: NSLocalizedString("", comment: ""), and: "")
                        UserDefaults.standard.setValue("", forKey: keyValue.del.rawValue)
                        _ = deletGroupInfoResultGroupDetail(entityName: Entities.resultGroupAnimalsTblEntity, groupName:group , customerId: self.customerId ?? 0, onFarmId: onFarmId, officalId: dataObject.officalId ?? "")
                        self.myHerdArray = fetchGroupInfoResult(entityName: Entities.resultGroupAnimalsTblEntity,groupName: group,customerId:self.customerId ?? 0) as! [ResultGroupsAnimals]
                        updateMyHerdDataStaus(entity: Entities.resultMyherdDataTblEntity, customerId:  self.customerId ?? 0, animalID: animalId , status: LocalizedStrings.inactiveStatus)
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let onformid = myHerdArray[indexPath.item].onFarmId
        let officalid = myHerdArray[indexPath.item].officalId
        let groupstatus = myHerdArray[indexPath.item].groupStatus
        let grouptypename = myHerdArray[indexPath.item].groupTypeName
        let animalidgroup = myHerdArray[indexPath.item].animalID
        selectedMyHerdArray = self.myHerdArray[indexPath.item]
        ...
        if searchByAnimal {
            ...
            vc?.newbackscreenindex = indexPath.item
            ...
            UserDefaults.standard.set(indexPath.item, forKey: keyValue.newBackScreenIndex.rawValue)
...
extension GroupDeetailsVCiPad:offlineCustomView{
    func crossBtnCall() {
...
extension GroupDeetailsVCiPad:UITextFieldDelegate{
...
extension GroupDeetailsVCiPad: scannedOCRProtocol {
...
extension GroupDeetailsVCiPad : chosseProviderDelegate {
...
extension GroupDeetailsVCiPad : SideMenuUI, RFID, nearByDevice {
...
extension GroupDeetailsVCiPad : swipeCell {
...
extension GroupDeetailsVCiPad : objectPickCartScreen {
