...
class CustomerOrderSetting {
    ...
    func saveCustomerSetting() {
        ...
        if let scanner =  UserDefaults.standard.value(forKey: keyValue.beefScannerSelection.rawValue) as? String,
            (UserDefaults.standard.value(forKey: keyValue.name.rawValue) as? String  == marketNameType.Beef.rawValue && (UserDefaults.standard.integer(forKey:keyValue.beefPvid.rawValue) == 5)) {
