        ...
        if UIDevice().userInterfaceIdiom == .phone {
            if indexPath.section == set.count {
                ...
                if UserDefaults.standard.value(forKey: keyValue.name.rawValue) as? String  != marketNameType.Dairy.rawValue{
                    ...
                    cell.dealerCodeView.backgroundColor = UIColor(displayP3Red: 1, green: 224/255, blue: 203/255, alpha: 1.0)
                ...
                if UserDefaults.standard.value(forKey: keyValue.name.rawValue) as? String  == marketNameType.Dairy.rawValue{
                    ...
                    if UserDefaults.standard.value(forKey: keyValue.submitTypeSelection.rawValue) as? String == keyValue.emailMe.rawValue || UserDefaults.standard.value(forKey: keyValue.submitTypeSelection.rawValue) as? String == "" ||  UserDefaults.standard.value(forKey: keyValue.submitTypeSelection.rawValue) as? String == nil{
                        ...
                        else  if !cell.emailMeSelectionOutlet.isSelected {
                        ...
                        else  if !cell.placeAnOrderSelectionOutlet.isSelected {
                    ...
                    if pviduser == 1 || pviduser == 2 || pviduser == 3 || pviduser == 10 || pviduser == 11 || pviduser == 12 {
                        if fetchAllDataAnimalDatarderIdNil(entityName: Entities.animalAddTblEntity,userId:userId,orderId:orderId,orderStatus:"false",farmid:"",animalTag:"",pvid: pviduser).count == 0 {
                            ...
                            if pviduser == 3 {
                                if UserDefaults.standard.value(forKey: keyValue.isAuSelected.rawValue) as? String == keyValue.noNeedAuPopUp.rawValue && !UserDefaults.standard.bool(forKey: keyValue.submitBtnFlag.rawValue) {
                                    cell.placeAnOrderSelectionOutlet.alpha = 1
                                    cell.placeAnotrderTitle.alpha = 1
                                }
                                else {
                ...
                if UserDefaults.standard.value(forKey: keyValue.name.rawValue) as! String  == marketNameType.Beef.rawValue{
                    if uniqueProductArrayBeef.count > 0{
                        ...
                        if UIDevice().userInterfaceIdiom == .phone {
                            ...
                            let addonsArrayBeef = (fetchSubProductDataTrueBeef(entityName:Entities.subProductBeefTblEntity,productId: (Int(cell.ProductArrayBeef[indexPath.row].productId)),animalTag: cell.ProductArrayBeef[indexPath.row].animalTag ?? "", status: "true",orderId:Int(cell.ProductArrayBeef[indexPath.row].orderId),userId:Int(cell.ProductArrayBeef[indexPath.row].userId)) as? [SubProductTblBeef])!
                                if (cell.ProductArrayBeef.count == 1 || cell.ProductArrayBeef.count == 2 ) && addonsArrayBeef.count == 0 {
                                    cell.collViewHeight.constant = CGFloat(height) + 40
                                }
                                else if addonsArrayBeef.count > 3  {
                                    cell.collViewHeight.constant = CGFloat(height) + 240
                                }
                                else {
                                    cell.collViewHeight.constant = CGFloat(height) + 140
                                }
                        }
                        cell.collView.reloadData()
                    }
                    ...
                    if uniqueProductArray.count > 0{
                        ...
                        if UIDevice().userInterfaceIdiom == .phone {
                            ...
                            if rem > 0{
                                ...
                                let addonsArray = ((fetchSubProductDataDairyreview(productId: Int(cell.ProductArray[indexPath.row].productId),animalTag: Int(cell.ProductArray[indexPath.row].animalId),orderId:Int(cell.ProductArray[indexPath.row].orderId),userId:Int(cell.ProductArray[indexPath.row].userId), status: "true") as? [SubProductTbl])!)
                                
                                if (cell.ProductArray.count == 1 || cell.ProductArray.count == 2 ) && addonsArray.count == 0 {
                                    cell.collViewHeight.constant = CGFloat(height) + 40
                                }
                                else if addonsArray.count > 3  {
                                    cell.collViewHeight.constant = CGFloat(height) + 240
                                }
                                else {
                                    cell.collViewHeight.constant = CGFloat(height) + 140
