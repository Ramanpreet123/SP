...
func fetchproviderProductDataGlobal( entityName: String,provId: Int,productId:Int, speciesName: String) -> NSArray{
    let appDelegate = UIApplication.shared.delegate as! AppDelegate
    let context = appDelegate.persistentContainer.viewContext
    let requestDel = NSFetchRequest<NSFetchRequestResult>(entityName: entityName)
    if speciesName != "" {
        requestDel.predicate = NSPredicate(format: "providerId == %d AND productId == %d AND speciesName == %@",provId,productId, speciesName)
    } else {
...
//  }
    ...
    //TODO: - remove
//    commanArray = [commanArray.firstObject, commanArray.firstObject, commanArray.firstObject, commanArray.firstObject]
    return commanArray
}
