...
extension ApiSyncList {
    ...
    func postListDataBeef(listId:Int64,custmerId:Int64,list_Name: String = "") {
      ...
      ApisyncList.updateAnimals.removeAll()
        ...
        ApisyncList.speciesId = SpeciesID.beefSpeciesId
        ApisyncList.providerId = beefpvid
        
        if UserDefaults.standard.integer(forKey: keyValue.beefPvid.rawValue) == 5  {
            if UserDefaults.standard.string(forKey: keyValue.beefProduct.rawValue) == keyValue.globalHD50K.rawValue {
                ApisyncList.productId = 19
            } else {
                ApisyncList.productId = 20
            }
        }
        if UserDefaults.standard.integer(forKey: keyValue.beefPvid.rawValue) == 6  {
            ...
            if UserDefaults.standard.string(forKey: keyValue.beefProduct.rawValue) == keyValue.genoTypeOnly.rawValue
            {
                ApisyncList.productId = 23
            }
            else if UserDefaults.standard.string(forKey: keyValue.beefProduct.rawValue) == keyValue.genoTypeStarBlack.rawValue
            {
                ApisyncList.productId = 999
            }
            else if UserDefaults.standard.string(forKey: keyValue.beefProduct.rawValue) == keyValue.genStarBlack.rawValue
            {
                ApisyncList.productId = 24
            }
        }
        if UserDefaults.standard.integer(forKey: keyValue.beefPvid.rawValue) == 7  {
            ApisyncList.productId = 25
        }
        if UserDefaults.standard.integer(forKey: keyValue.beefPvid.rawValue) == 13  {
            ApisyncList.productId = 52
        }
      ...
      if pvid == 13{
        ...
        if dataModel.customerList.animals.count < animaltbl.count {
            ...
            if duplicateAnimalData.count>0{
                ...
                self.deleteDuplicateAnimalsFromSever(animalList: duplicateAnimalData, list_id: listId, speciesID: SpeciesID.beefSpeciesId)
            }
        } else {
    ...
    func PostOffineDeleteanimals(species: String){
        ...
        for item in filteredAnimals {
            ...
            ApisyncList.listName = getListName
            ApisyncList.description = descriptionName
            ApisyncList.speciesId = species
            ApisyncList.providerId = providerId
            let jsonEncoder = JSONEncoder()
            let jsonData = try! jsonEncoder.encode(ApisyncList)
            let json = String(data: jsonData, encoding: String.Encoding.utf8)
            print(json!)
    ...
    func deleteDuplicateAnimalsFromSever(animalList : [AnimalResponse], list_id: Int = 0, speciesID :String = SpeciesID.dairySpeciesId){
        self.ApisyncList.addAnimals.removeAll()
        if speciesID ==  SpeciesID.dairySpeciesId {
            let fetchName = fetchListNameAccordingToListId(entityName: Entities.dataEntryListTblEntity,customerId:Int(UserDefaults.standard.value(forKey: keyValue.currentActiveCustomerId.rawValue) as! Int64 ),listId:Int64(list_id),providerId:UserDefaults.standard.integer(forKey: keyValue.providerID.rawValue))
            if fetchName.count != 0{
                let getName = fetchName.object(at: 0) as? DataEntryList
                self.getListName = getName?.listName ?? ""
                self.descriptionName = getName?.listDesc ?? ""
            ...
            let fetchName = fetchListNameAccordingToListId(entityName: Entities.dataEntryListTblEntity,customerId:Int(UserDefaults.standard.value(forKey: keyValue.currentActiveCustomerId.rawValue) as! Int64 ),listId:Int64(list_id),providerId:beefpvid)
            if fetchName.count != 0{
                let getName = fetchName.object(at: 0) as? DataEntryList
                self.getListName = getName?.listName ?? ""
                self.descriptionName = getName?.listDesc ?? ""
        ...
        ApisyncList.listName = getListName
        ApisyncList.description = descriptionName
        ApisyncList.speciesId = speciesID
        if speciesID ==  SpeciesID.dairySpeciesId {
        ...
        AF.request(request as URLRequestConvertible).responseJSON { response in
            ...
            case .success(_):
                ...
                self.ApisyncList.deleteAnimals.removeAll()
                if speciesID ==  SpeciesID.dairySpeciesId {
                    self.responseRecieved1(data, status: true, listId: Int(list_id))
                } else {
                    self.responseRecieved1Beef(data, status: true, listId: Int(list_id))
                }
